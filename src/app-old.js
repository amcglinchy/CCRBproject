...
function init() {
    ...
    let grid = d3.select("grid-id")
        .on("click", ()=>{
            if (chartType != 'grid'){
                chartType = 'grid'

                grid.classList.toggle('btn')
                grid.classList.toggle('btn-selected')
                races.classList.toggle('btn')
                races.classList.toggle('btn-selected')

                update(chartType)
            }
        })
        let races = d3.select("race-id")
        .on("click", ()=>{
            if (chartType != 'grid'){
                chartType = 'grid'
    ...
    // let grid = gridContainer.selectAll("circle")
    //     .data(state.data)
    //     .join("circle")
    //     .attr("class", "circle")
...
function draw() {
  ...
  // Tooltip
    ...
    let clickCounter = () => {
        clicks = clicks +1;
        console.log(clicks)}
    d3.select("#race-id").on('click', clickCounter);
}